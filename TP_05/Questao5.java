...
public class Questao5 {
    ...
    public static void main(String[] args) {
        ...
        // imprime os maiores preços
        int count = 0;
        for (int i = array.length - 1; i >= 0 && count < 5; i--) {
            // Condição mantida do código original
            if (array[i].getPrice() >= 0) {
                array[i].imprimir();
                count++;
            }
        }
        ...
        System.out.println("| 5 pre\u00E7os mais baratos |");    // ç = \u00E7

        // imprime os menores preços
        count = 0;
    ...
    private static void intercalar(Game[] array, Game[] jogos, int esq, int meio, int dir) {
        // Cópia para o array temporário (jogos)
        for (int i = esq; i <= dir; i++) {
            jogos[i] = array[i];
            // Contagem de Movimentação: Cópia para o array auxiliar
            movimentacao++;
        }
        ...
        // Intercalação e cópia de volta para o array principal (array)
        while (i <= meio && j <= dir) {
            ...
            // Contagem de Movimentação: Cópia do auxiliar para o principal
            movimentacao++;
        }

        // Cópia dos elementos restantes da esquerda
        while (i <= meio) {
            array[k++] = jogos[i++];
            // Contagem de Movimentação: Cópia do auxiliar para o principal
            movimentacao++;
        ...
        // Cópia dos elementos restantes da direita (mantido do original)
        while (j <= dir) {
            array[k++] = jogos[j++];
            // Contagem de Movimentação: Cópia do auxiliar para o principal
            movimentacao++;
    ...
    // Compara por price e desempate por id
    ...
    private static void gerarLog() {
        try {
            PrintWriter writer = new PrintWriter("1526815_mergesort.txt", "UTF-8");
            long tempoExecucao = System.currentTimeMillis() - tempoInicio;
            writer.println("1526815\t" + comparacao + "\t" + movimentacao + "\t" + tempoExecucao);
            writer.close();
...
class Game {
    ...
    // Leitura do objeto Game a partir de uma linha CSV
    ...
    // Método para remover espaços em branco do início e fim
    ...
    // Método para aplicar trim em um array de strings
    ...
    // Método para comparar duas strings
    ...
    public void imprimir() {
        ...
        System.out.print(this.estimatedOwners + " ## " + String.format(Locale.US, "%.2f", this.price) + " ## ");
        imprimirArray(supportedLanguages);
        System.out.print(
                " ## " + this.metacriticScore + " ## " + String.format(Locale.US, "%.1f", this.userScore) + " ## ");
